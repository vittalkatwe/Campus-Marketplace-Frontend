...
import { ItemDto } from '@/constants/types';
...
export const api = {
  ...
  getItemsByCategory: async (category: string): Promise<Item[]> => {
    const res = await fetch(`${API_BASE_URL}/api/items/category/${category}`);
    if (!res.ok) throw new Error(await res.text());
    return res.json();
  },
  
  getUserItems: async (token: string): Promise<Item[]> => {
    const res = await fetch(`${API_BASE_URL}/api/items/user/me`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) throw new Error(await res.text());
    return res.json();
  ...
  addItem: async (item: ItemDto, token: string): Promise<Item> => {
    const res = await fetch(`${API_BASE_URL}/api/items`, {
      method: 'POST',
      headers: {
